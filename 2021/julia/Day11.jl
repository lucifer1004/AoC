...
# ╔═╡ 7db5a971-c5bb-4e9e-9822-69a42a04d82c
function go!(board::Vector{Vector{Int64}})
	n = length(board)
	m = length(board[1])
...
function part_one(input)
	lines = split(input, "\n")
	board = map(x -> map(ch -> parse(Int, ch), collect(x)), lines)
	day = 100
	ans = 0
	for _ in 1:100
		ans += go!(board)
...
function part_two(input)
	...
	while true
		day += 1
		tick = go!(board)
		if tick == length(board) * length(board[1])
			return day
		end
	end
