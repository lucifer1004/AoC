...
md"Day: $(@bind day NumberField(0:25, default=22))"
...
# ╔═╡ 97b2c655-0e71-47b9-b911-1e16dfc28d97
...
begin
	...
	function parse_line(line::AbstractString)
		state, range = split(line, " ")
		xs, xe, ys, ye, zs, ze = map(x -> parse(Int, x), match(regex, range))
		return (state, xs, xe, ys, ye, zs, ze)
	end
	
	function parse_input(input::AbstractString)
		map(parse_line, split(input, "\n"))
	end
...
# ╔═╡ f6e7ba92-a226-4512-a811-1ac5940a7df3
begin
	mutable struct KDNode
		range::Vector{Tuple{Int64, Int64}}
		children::Vector{KDNode}
		state::Bool
	end

	function set_state!(node::KDNode, range::AbstractVector{Tuple{Int64, Int64}}, state::Bool)		
		contained = all(x -> x[2][1] <= x[1][1] < x[1][2] <= x[2][2], zip(node.range, range))
		outside = any(x -> x[2][1] >= x[1][2] || x[2][2] <= x[1][1], zip(node.range, range))
		...
		elseif length(node.children) == 0
			sub = []
			for ((l, r), (l1, r1)) in zip(node.range, range)
				pts = sort(unique(filter(x -> l <= x <= r, [l, r, l1, r1])))
				m = length(pts)
				push!(sub, collect(zip(pts[1:m - 1], pts[2:m])))
			end
			for x in product(sub...)
				push!(node.children, KDNode(collect(x), [], node.state))
			end
		end

		for child in node.children
			set_state!(child, range, state)
		end
	end

	function calc(node::KDNode)
		if length(node.children) == 0
			return node.state ? prod(map(x -> x[2] - x[1], node.range)) : 0
		else
			return sum(map(x -> calc(x), node.children))
		end
	end

	md"""KDNode defined here."""
end
...
function solve(input)
	...
	root = KDNode(repeat([(-inf, inf + 1)], k), [], false)
	for (state, xs, xe, ys, ye, zs, ze) in input
		set_state!(root, [(xs, xe + 1), (ys, ye + 1), (zs, ze + 1)], state == "on")
	end
...
function part_one(input)
	input = filter(x -> minimum([x[2], x[4], x[6]]) >= -50 && maximum([x[3], x[5], x[7]]) <= 50, input)
