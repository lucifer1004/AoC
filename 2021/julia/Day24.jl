...
# The solution is hard-coded.
function solve(input, order)
	a = map(i -> parse(Int, split(input[(i - 1 ) * 18 + 6], " ")[3]), 1:14)
	b = map(i -> parse(Int, split(input[(i - 1 ) * 18 + 16], " ")[3]), 1:14)
	c = map(i -> Int(parse(Int, split(input[(i - 1 ) * 18 + 5], " ")[3]) == 26), 1:14)
	...
	return join(map(string, ans))
end

# ╔═╡ 84d1ee32-e4fc-4ccb-ad7a-91d359f762ba
function part_one(input)
	return solve(input, 9:-1:1)
end
