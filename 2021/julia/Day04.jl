...
# ╔═╡ af5418e4-72ff-4914-ba56-4142bd5ab1ce
function get_board(board_str)
	return map(row -> map(x -> parse(Int, x), filter(x -> length(x) > 0, split(row, " "))), board_str)
end

# ╔═╡ 144e8398-0cf8-4eea-b15b-302415e296da
function get_win(board, mp)
	n = length(board)
	return min(minimum(maximum(mp[board[i][j]] for j in 1:n) for i in 1:n), minimum(maximum(mp[board[j][i]] for j in 1:n) for i in 1:n))
end

# ╔═╡ e20707ef-06bb-4cfb-95d5-603e0afe5ed2
function solve(input, rnk)
	lines = split(input, "\n")
	n = length(lines)
	m = n ÷ 6
	...
	nums = map(x -> parse(Int, x), split(lines[1], ","))
	boards = map(get_board, [lines[i:i+4] for i in 3:6:n])
	mp = Dict(num => i for (i, num) in enumerate(nums))
	win = map(board -> get_win(board, mp), boards)
	order = Array(1:m)
	sort!(order, by = x -> win[x])
	no = order[rnk]
	turn = win[no]

	x = nums[turn]
	s = Set(nums[1:turn])
	rem = sum(sum(x for x in row if x ∉ s) for row in boards[no])
	return x * rem
end
