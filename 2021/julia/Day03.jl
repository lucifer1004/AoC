...
function part_one(input)
	...
	half = (n + 1) ÷ 2
	m = length(codes[1])
	x = sum(if count(ch -> ch == '1', [code[i] for code in codes]) >= half 1 << (m - i) else 0 end for i in 1:m)
	y = sum(if count(ch -> ch == '0', [code[i] for code in codes]) <= n - half 0 else 1 << (m - i) end for i in 1:m)
	return x * y
end
...
function part_two(input)
	...
	n = length(codes)
	m = length(codes[1])
	remaining = copy(codes)
	for i in 1:m
		hi_half = (length(remaining) + 1) ÷ 2
		hi = if count(code -> code[i] == '1', remaining) >= hi_half '1' else '0' end
		remaining = filter(code -> code[i] == hi, remaining)
	end
	o2 = parse(Int, remaining[1], base=2)
	remaining = copy(codes)
	for i in 1:m
		lo_half = length(remaining) ÷ 2
		lo = if count(code -> code[i] == '0', remaining) <= lo_half '0' else '1' end
		remaining = filter(code -> code[i] == lo, remaining)
		if length(remaining) == 1
			break
		end
	end
	co2 = parse(Int, remaining[1], base=2)
	return o2 * co2
end
