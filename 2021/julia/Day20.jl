...
# ╔═╡ 7b37ef05-af9c-4fa3-9b01-20baf3196faa
...
# ╔═╡ ab8a9128-5041-4406-b014-ef4b962961b7
function solve(input, times)
	lines = split(input)
	alg = lines[1]
	n = length(lines) - 1
	m = length(lines[2])
	board = Dict((i, j) => lines[i + 1][j] for i in 1:n for j in 1:m)

	for _ in 1:times
		new_board = Dict()
		for i in 1 - 2times:n + 2times
			for j in 1 - 2times:m + 2times
				it = 8
				id = 0
				for di in -1:1
					for dj in -1:1
						ni = i + di
						nj = j + dj
						if (ni, nj) ∈ keys(board) && board[(ni, nj)] == '#'
							id += 1 << it
						end
						it -= 1
					end
				end
				new_board[(i, j)] = alg[id + 1]
			end
		end
		board = new_board
	end

	count(x -> x[2] == '#' && 1 - times <= x[1][1] <= n + times && 1 - times <= x[1][2] <= m + times, board)
end
