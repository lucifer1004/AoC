...
function part_one(input)
	...
	function get_neighbors(r, c)
		return filter(x -> 1 <= x[1] <= n && 1 <= x[2] <= m, [(r - 1, c), (r, c - 1), (r + 1, c), (r, c + 1)])
	end

	ans = 0
	for i in 1:n
		for j in 1:m
			lo = minimum(lines[r][c] for (r, c) in get_neighbors(i, j))
			if lo > lines[i][j]
				ans += 1 + parse(Int, lines[i][j])
			end
		end
	end
