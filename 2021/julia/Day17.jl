...
# ╔═╡ b91d548f-9509-4b7b-885e-4e49a8d1b9bf
regex = r"""target area: x=(-?\d+)..(-?\d+), y=(-?\d+)..(-?\d+)"""

# ╔═╡ babe477a-9a83-4e94-9976-0a65f2209cbc
function solve(l::Int, r::Int, d::Int, u::Int)
	vxₘᵢₙ = min(l, 0)
	vxₘₐₓ = max(r, 0)
	vyₘᵢₙ = min(d, 0)
	vyₘₐₓ = max(abs(d), abs(u))

	valid_path = 0
	yₘₐₓ = 0
	for vx₀ in vxₘᵢₙ:vxₘₐₓ
		for vy₀ in vyₘᵢₙ:vyₘₐₓ
			collision = false
			vx = vx₀
			vy = vy₀
			x = 0
			...
			if collision
				valid_path += 1
				yₘₐₓ = max(yₘₐₓ, vy₀ * (vy₀ + 1) ÷ 2)
			end
		end
	end

	return valid_path, yₘₐₓ
end

# ╔═╡ 84d1ee32-e4fc-4ccb-ad7a-91d359f762ba
function part_one(input::AbstractString)
	return solve(map(x -> parse(Int, x), match(regex, input).captures)...)[2]
end
...
function part_two(input::AbstractString)
	return solve(map(x -> parse(Int, x), match(regex, input).captures)...)[1]
end
