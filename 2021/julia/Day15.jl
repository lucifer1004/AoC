...
function solve(source, target, cost_fn, is_valid)
	d = Dict()
	d[source] = 0
	...
	while length(pq) > 0
		(i, j), dis = dequeue_pair!(pq)
		for (di, dj) in [(-1, 0), (0, -1), (0, 1), (1, 0)]
			ni = i + di
			nj = j + dj
			if is_valid(ni, nj)
				nd = dis + cost_fn(ni, nj)
				if (ni, nj) ∉ keys(d) || nd < d[(ni, nj)]
					d[(ni, nj)] = nd
					pq[(ni, nj)] = nd
				end
			end
...
function part_one(input)
	a = map(x -> [parse(Int, ch) for ch in x], split(input))
	n = length(a)
	m = length(a[1])
	source = (1, 1)
	target = (n, m)
	cost_fn = (i, j) -> a[i][j]
	is_valid = (i, j) -> 1 <= i <= n && 1 <= j <= m
	return solve(source, target, cost_fn, is_valid)
end
...
function part_two(input)
	...
	source = (1, 1)
	target = (n * 5, m * 5)
	cost_fn = (i, j) -> (a[(i - 1) % n + 1][(j - 1) % m + 1] - 1 + (i - 1) ÷ n + (j - 1) ÷ m) % 9 + 1
	is_valid = (i, j) -> 1 <= i <= n * 5 && 1 <= j <= m * 5
	return solve(source, target, cost_fn, is_valid)
end
