...
md"Day: $(@bind day NumberField(0:25, default=21))"
...
md"""
sample = $(@bind sample TextField((80, 5); default="Player 1 starting position: 4\nPlayer 2 starting position: 8"))
"""
...
begin
	function parse_input(input::AbstractString)
		return map(x -> parse(Int, split(x, ": ")[2]), split(input, "\n"))
	end
...
function part_two(input)
	a, b = input
	dp = zeros(Int64, 10, 10, 22, 22, 2)
	dp[a, b, 1, 1, 1] = 1
	coeff = [0, 0, 1, 3, 6, 7, 6, 3, 1]
					...
					for b in 1:10
						...
						if q == 1
							for die in 3:9
								a′ = (a + die - 1) % 10 + 1
								pa′ = min(22, pa + a′)
								dp[a′, b, pa′, pb, 2] += dp[a, b, pa, pb, 1] * coeff[die]
							end
						else
							for die in 3:9
								b′ = (b + die - 1) % 10 + 1
								pb′ = min(22, pb + b′)
								dp[a, b′, pa, pb′, 1] += dp[a, b, pa, pb, 2] * coeff[die]
							end
						end
					end
				end
